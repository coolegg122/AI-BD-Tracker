...
export default function Pipeline() {
  const { deals, stages, openDealOverview } = useStore();
  const [activeTab, setActiveTab] = useState('by_deal'); // 'by_deal' or 'by_company'
  ...
  // Process data for View 1: By Internal Deal
  const groupedByDeal = useMemo(() => {
    const groups = {};
    deals.forEach(d => {
      const pipeline = d.pipeline || 'General Engagement';
      if (!groups[pipeline]) groups[pipeline] = [];
      groups[pipeline].push(d);
    });
    return groups;
  }, [deals]);
  ...
  const groupedByCompany = useMemo(() => {
    const groups = {};
    deals.forEach(d => {
      const company = d.company || 'Unknown Company';
  ...
  const getFeedbackSummary = (d) => {
    if (d.tasks && d.tasks.length > 0) {
      return d.tasks[0].desc || d.tasks[0].title;
    }
    if (d.nextFollowUp) {
      return `Waiting for follow-up on ${d.nextFollowUp}`;
    }
        ...
        <div className="flex justify-between items-end mb-6">
          <div>
            <nav className="flex items-center gap-1.5 text-[10px] font-bold text-ui-text-muted mb-2 uppercase tracking-widest">
              ...
              <span className="text-ui-accent">Engagement Matrix</span>
            </nav>
          ...
          <div className="bg-ui-hover p-1 rounded-xl flex text-sm font-bold shadow-inner transition-colors">
            <button 
              onClick={() => setActiveTab('by_deal')}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg transition-all duration-200 ${
                activeTab === 'by_deal' ? 'bg-ui-card text-ui-accent shadow-sm' : 'text-ui-text-muted hover:text-ui-text'
              }`}
            >
              <Layers className="w-4 h-4" /> By Internal Deal
            </button>
            <button 
      ...
      <div className="flex-1 overflow-y-auto pb-12 pr-2">
        ...
        {deals.length === 0 && (
           <div className="text-center py-20 bg-ui-card rounded-2xl border border-ui-border shadow-sm transition-colors">
             ...
             <h3 className="text-lg font-bold text-ui-text">No deals actively tracked</h3>
             <p className="text-ui-text-muted text-sm mt-2">Extract BD memos via Smart Input to populate the matrix.</p>
           </div>
        )}

        {/* =========================================
            VIEW 1: GROUPED BY INTERNAL DEAL (PIPELINE)
            ========================================= */}
        {activeTab === 'by_deal' && Object.entries(groupedByDeal).map(([pipelineName, dealList]) => {
          const isOpen = openSections[`deal_${pipelineName}`] !== false; // default open
          return (
            <div key={pipelineName} className="mb-6 bg-ui-card rounded-2xl border border-ui-border shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 transition-colors">
              <button 
                onClick={() => toggleSection(`deal_${pipelineName}`)}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs font-bold text-ui-text-muted uppercase tracking-widest mt-0.5">
                      Managing {dealList.length} Partner Engagement{dealList.length > 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
              ...
              {isOpen && (
                <div className="p-6">
                  {stages.map(stage => {
                    const stageDeals = dealList.filter(d => (d.stage || 'Initial Contact') === stage.id);
                    ...
                    return (
                      <div key={stage.id} className="mb-8 last:mb-0 relative">
                        <div className="flex items-center gap-3 mb-4 sticky top-0 bg-ui-card z-10 py-1 transition-colors">
                           ...
                           <span className="text-[10px] font-bold text-ui-text-muted bg-ui-bg px-2 py-0.5 rounded-full">{stageDeals.length}</span>
                        </div>
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-1">
                                  <div className="flex items-center gap-3">
                                    <h5 className="font-bold text-ui-text group-hover:text-ui-accent transition-colors truncate max-w-[120px]" title={d.company}>{d.company}</h5>
                                    <button 
                                      onClick={(e) => { e.stopPropagation(); setIntelligenceCompany(d.company); }}
                                  ...
                                  <span className="text-[10px] font-bold text-ui-text-muted whitespace-nowrap ml-2"><Clock className="w-3 h-3 inline mr-1 -mt-0.5"/>{d.lastContactDate || 'Recently'}</span>
                                </div>
        ...
        {/* =========================================
            ...
            ========================================= */}
        {activeTab === 'by_company' && Object.entries(groupedByCompany).map(([companyName, dealList]) => {
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left flex-1">
                    ...
                    <p className="text-xs font-bold text-ui-accent uppercase tracking-widest mt-0.5 transition-colors">
                      {dealList.length} Interfacing Track{dealList.length > 1 ? 's' : ''}
                    </p>
                  </div>
                 </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-ui-bg border-b border-ui-border text-[10px] font-bold text-ui-text-muted uppercase tracking-widest transition-colors">
                        <th className="p-4 pl-6 w-1/4">Internal Deal</th>
                      ...
                      {dealList.map(d => (
                        ...
                        >
                          <td className="p-4 pl-6 align-top">
                            <div className="font-bold text-sm text-ui-text">{d.pipeline}</div>
                            <div className="text-[10px] font-bold text-ui-text-muted mt-1 uppercase">ID: DEAL-{d.id}</div>
                          </td>
                          <td className="p-4 align-top">
                            <div className="flex items-center gap-2">
                              <span className={`w-2 h-2 rounded-full ${getStageColor(d.stage || 'Initial Contact')}`}></span>
                              <span className="text-xs font-bold text-ui-text">{d.stage || 'Initial Contact'}</span>
                            </div>
                          </td>
                          <td className="p-4 align-top">
                          ...
                          <td className="p-4 pr-6 align-top">
                            {d.nextFollowUp ? (
                              <div className="flex flex-col gap-1">
                                <span className="text-xs font-bold text-ui-text transition-colors">{d.nextFollowUp}</span>
